...
import { buildHoles, buildRows, buildWinPocket, flickDirX, notchPos } from '../src/game/board.ts';
import { createCoin, flickCoin, placeOnRow, stepCoin } from '../src/game/coin.ts';
  ...
  for (const r of rows) {
    console.log(
      `    段${r.index + 1}: 板 x=${r.left.toFixed(0)}..${r.right.toFixed(0)}  ` +
        `溝=${r.notchSide === 'right' ? '右' : '左'}  y=${r.notchY}`,
    );
  }
...
for (const d of Object.values(DIFFICULTIES)) {
  ...
  for (let i = 0; i < ROW_COUNT; i++) {
    ...
    jumps.push([Math.abs(from.x - target.near), target.y - rows[i]!.notchY]);
  }
  const uniform = jumps.every(
    (j) => Math.abs(j[0] - jumps[0]![0]) < 0.5 && Math.abs(j[1] - jumps[0]![1]) < 0.5,
  );
  console.log(
    `  ${d.label}: 手前の穴を越える横距離=${jumps[0]![0].toFixed(0)}  落差=${jumps[0]![1].toFixed(0)}`,
  );
  check(`${d.label}: 5つの遷移がすべて同一条件`, uniform);
...
for (const d of Object.values(DIFFICULTIES)) {
  ...
  const { placeAtStart, canFlick } = await import('../src/game/coin.ts');
...
for (const d of Object.values(DIFFICULTIES)) {
  ...
  for (let row = 0; row < ROW_COUNT && !escaped; row++) {
    for (let power = P_MIN; power <= P_MAX && !escaped; power += 25) {
      const coin = createCoin();
      placeOnRow(coin, rows, row, notchPos(rows[row]!).x);
...
check('弾きゾーンが手前の穴より狭い', FLICK_ZONE_PX < NEAR_GAP);
