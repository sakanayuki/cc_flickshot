...
| 7 | **公開したら画面が真っ青で何も出なかった。** `base` を `'/cc_flickshot/'` と絶対パスで固定していたため、配信パスが合わず JS が 404。JS が動かないと body の背景色だけが見える状態になり、エラーも出ないので原因が分からない | `base: './'` の相対パスに変更。あわせて、起動や描画で例外が出たら画面に理由を表示するようにした | §14.2 |
...
export default defineConfig({
  base: './',               // 相対パス。理由は下記
  build: { target: 'es2020', outDir: 'dist', modulePreload: false },
});
```

**`base` は絶対パス(`'/cc_flickshot/'`)にしてはならない。**
配信されるパスが少しでも違うと JS が 404 になり、画面が body の背景色だけの
**真っ青な状態**になる。エラーも何も出ないため原因がまったく分からない。
相対パスならどのパスに置いても動く。

`modulePreload: false` は、単一バンドルで動的 import がないため不要なポリフィルを
除くもの。入れておくと `fetch()` がバンドルに混ざり、「初回ロード以外ネットワーク
不要」という要件が読み取りにくくなる。
...
- [x] どのパスに配信しても動く(`base: './'`。`/` と `/cc_flickshot/` の両方で描画を確認)
- [x] 起動・描画で例外が出たら画面に理由が表示される(真っ青な画面のまま原因不明にならない)
