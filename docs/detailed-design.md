...
重力 `g = 2200 px/s²`、弾き角 20°(鉛直から盤面内側)として、横150px流れる間に145px上昇するのに必要な初速は:

```
v_min = 904 px/s   (安全マージン8px込み。幾何的下限は 890 px/s)
```

`P_MIN = 620`、`P_MAX = 1700` とすると `pull = (904-620)/(1700-620) = 0.263`。
**pull ≥ 0.27 でどの段からでも次段に到達する。ストロークの74%が成功域。**

ゴール成功域(バスケット左端との関係):

| 難易度 | バスケット左端 | 成功する pull の範囲 | ストローク比 |
|---|---|---|---|
| やさしい | 280 | 0.26 〜 1.00 | 74% |
| ふつう | 385 | 0.26 〜 0.78 | 51% |

やさしいは「弱すぎ」でしか外れず、ふつうは「強すぎ」でも外れる。発注者の「両方とも外れることがある」を満たす。

> 検算スクリプトは §13.5 に再現手順を残す。座標や `g` を変更したら必ず再計算し、この表を更新すること。
...
| `fallSpeed` | 230 | 320 | 大きいほど落ちやすい |
| `goalBasketLeft` | 280 | 385 | 小さいほどゴールが広い |
| `lipEscapeSpeed` | `null` | 520 | 転落の有無 |

**弾き力(`P_MIN`/`P_MAX`)・ストローク・弾きゾーン・穴以外の盤面形状は両難易度で共通。**

### 7.2 数値の設計根拠

- `fallSpeed`: 弱い弾きで戻ったコインは約247 px/s でレバー端へ向かう。やさしい(230)ならこれを **かろうじて通過**、ふつう(320)なら **落ちる**。一方、成功した弾きから着地したコインは約493 px/s なので、どちらの難易度でも通過する
- `goalBasketLeft`: §3.6 の表のとおり、やさしいはゴール成功域が pull 0.26〜1.00(74%)、ふつうは 0.26〜0.78(51%)になる
- `lipEscapeSpeed = 520`: 通常の転がり(最大322 px/s)では発生せず、着地直後の高速状態からさらに加速した場合のみ転落する
...
| 時刻 | 内容 |
|---|---|
| 0.0 – 0.4s | コインが画面外(上)から投入口へ落ちてくる |
| 0.4 – 0.5s | 投入口で「コトン」と沈む(スケールを一瞬つぶす) |
| 0.5 – 1.3s | 投入チューブ (185,900)→(150,702) に沿って上昇。チューブは半透明で中のコインが見える |
| 1.3 – 1.5s | 段1の高い端 (210, 702) に出て、`onLane` として `s = 0` `vs = 0` で配置 |
| 1.5s | `play` へ |
...
export const FLICK_ANGLE_DEG = 20;      // 鉛直から盤面内側へ
...
export const DIFFICULTIES: Record<'easy' | 'normal', DifficultyConfig> = {
  easy: {
    ...
    goalBasketLeft: 280, lipEscapeSpeed: null,
  },
  normal: {
    id: 'normal', label: 'ふつう',
    holeS: [[0.85], [0.85], [0.65, 0.87], [0.65, 0.87], [0.65, 0.87]],
    holeRadius: 34, fallSpeed: 320,
    goalBasketLeft: 385, lipEscapeSpeed: 520,
  },
};
```
...
export const LANES: Lane[] = Array.from({ length: LANE_COUNT }, (_, i) => {
  ...
  const loY = BOARD_BOTTOM - 8 - i * LANE_GAP;          // 段1のレバー端 y = 742
  const hiY = loY - LANE_DROP;
  return leverRight
    ? { index: i, hi: { x: BOARD_LEFT + SHAFT_W,  y: hiY }, lo: { x: BOARD_RIGHT, y: loY }, leverSide: 'right' }
    : { index: i, hi: { x: BOARD_RIGHT - SHAFT_W, y: hiY }, lo: { x: BOARD_LEFT,  y: loY }, leverSide: 'left'  };
});
```
...
**[`docs/verify-geometry.py`](verify-geometry.py) を同梱している。** 本書のすべての数値はこのスクリプトで検証済みで、13項目すべてが通ることを確認してある。
...
| 2 | **5つの跳躍がすべて「横150px・上145px」で完全に均一**(§3.1) |
| 3 | 必要初速 `v_min` と、`P_MIN`/`P_MAX` に対する成功 `pull` の範囲 |
| 4 | 各難易度のゴール成功域。かつ「外す余地」が残っていること |
| 5 | 転がり速度と `fallSpeed` / `lipEscapeSpeed` の関係が §7.2 の設計意図どおりか |
| 6 | 弾きゾーンがシャフトの内側に収まっているか(§6.5) |
| 7 | 盤面・ノブ・ストロークが 1280px に収まるか(§2.3, §6.3) |

**実装時は同じ内容を `scripts/verify-geometry.ts` に移植し、`npm run verify` で実行できるようにする。**
座標・`GRAVITY`・`FLICK_ANGLE_DEG`・`P_MIN`/`P_MAX`・難易度パラメータのいずれかを変更したら必ず実行し、失敗した項目があれば §3.6 の表を更新するか、変更を取り消すこと。
...
plan.md §9 を、本書の章番号に紐付けて具体化したもの。

- [ ] スマホ縦持ちで盤面全体とプランジャーが1画面に収まる(§2.3)
- [ ] 指のドラッグ量450pxで `pull` が 0→1 まで変化し、ノブは画面内に収まっている(§6.3)
- [ ] 横方向にドラッグが蛇行しても `pull` に影響しない(§6.1)
- [ ] 発射で5段すべてのレバーが同時に動く(§6.5)
- [ ] 5段どこからでも `pull ≥ 0.27` で次の段に到達する(§3.6, §13.4)
- [ ] 最大まで引いても失敗にならず、上の段に着地する(§4.3(b))
- [ ] メダル投入アニメ → プレイ → (穴 / ゴール / あきらめ) → リザルト がすべて機能する(§8.3)
- [ ] あきらめるは1秒長押しでのみ発動し、指がずれるとキャンセルされる(§8.6)
- [ ] 2秒放置で指アイコンが毎回表示され、触れると即消える(§8.5)
- [ ] ゴールを外してもゲームは終わらず、段5から再挑戦できる(§5.3)
- [ ] やさしいは適当に引いても20回中16回以上ゴールできる(§7.4)
- [ ] ふつうは大人が狙って20回中5〜8回程度ゴールできる(§7.4)
- [ ] ゴールでスタンプが増え、リロード後もタイトル画面に残っている(§10)
- [ ] プライベートブラウズでもクラッシュせず遊べる(§10)
- [ ] 音は一切鳴らない(`AudioContext` を生成するコードが存在しない)
- [ ] 外部素材(画像・音・フォント・CDN)への参照が一切ない(§9)
- [ ] `npm test` が通る(§13)
- [ ] `https://sakanayuki.github.io/cc_flickshot/` で動作し、main への push で自動デプロイされる(§14)
